...
void AudioSystem::loadSound(std::string_view sound_name, std::string_view file_path) noexcept {
    ...
    if (_sounds.find(std::string(sound_name)) != _sounds.end()) {
    ...
    _sounds.emplace(sound_name, sound);
    DULL_INFO("Sound loaded successfully: {}", sound_name);
}

void AudioSystem::playSound(std::string_view sound_name) noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
...
void AudioSystem::loadMusic(std::string_view music_name, std::string_view file_path) noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    if (_musics.find(std::string(music_name)) != _musics.end()) {
        err.failFallback("Name already exist");
        return;
    }
    ...
    _musics.emplace(music_name, music);
    DULL_INFO("Music loaded successfully: {}", music_name);
}

void AudioSystem::playMusic(std::string_view music_name) noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
