...
#define ZVALIDATE(CONDITION, TAG, KILL) do {                            \
  if (!(CONDITION)) {                                                   \
    auto color = zutils::internal::ColorText{ (KILL ? 41 : 31), TAG };  \
    ZLOGF << color << " : " << __LINE__ << " : " #CONDITION;            \
    if constexpr (KILL) zutils::internal::killProcess();                \
  }                                                                     \
} while (0)
