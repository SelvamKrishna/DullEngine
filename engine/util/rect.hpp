...
/// --- Constructors ---
    ...
    [[nodiscard]]
    constexpr Rect(const rl::Rectangle& rl_rect) noexcept
    : x{rl_rect.x}, y{rl_rect.y}, w{rl_rect.width}, h{rl_rect.height} {}

/// --- Accessors ---

    [[nodiscard]]
    constexpr Vec2f getPosition() const noexcept { return {x, y}; }

    [[nodiscard]]
    constexpr Vec2f getDimension() const noexcept { return {w, h}; }
...
/// --- Collision ---
    ...
    [[nodiscard]]
    bool containsPoint(const Vec2f& point) const noexcept
    {
        return rl::CheckCollisionPointRec(point, *this);
    }

    [[nodiscard]]
    bool intersectsRec(const Rect& other) const noexcept
    {
